...
test("renders the portfolio with its primary SEO metadata", async () => {
  ...
  assert.match(html, /id="project-medical-platform"/i);
  assert.match(html, /id="practice-invoice-dashboard"/i);
  assert.match(html, /mailto:soriahaidary17@gmail\.com/i);
  assert.match(html, /linkedin\.com\/in\/suriya-haidari/i);
});
